// Looper Pedal Board – Phase-Locked Multi-Track Version
...
// ====== IMPROVED PHASE-LOCKED LOOPER CLASS ======
class Looper {
  ...
  async phaseLockedRecording() {
    ...
    setTimeout(() => {
      this._startPhaseLockedRecording(targetLen);
    }, timeToNextBoundary * 1000);
  }

  async _startPhaseLockedRecording(targetLen) {
    ...
    function animateRec() {
      if (self.state === 'recording') {
        let elapsed = (Date.now() - startTime) / (targetLen * 1000);
    ...
    setTimeout(() => {
      if (self.state === 'recording') self.stopRecordingAndPlay();
    }, targetLen * 1000);
  }
